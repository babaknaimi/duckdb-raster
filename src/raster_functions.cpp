...
//  cell_id.
static void RasterPixelValueFunction(DataChunk &input, ExpressionState &, Vector &result) {
    auto count = input.size();
    result.SetVectorType(VectorType::FLAT_VECTOR);
    for (idx_t row_idx = 0; row_idx < count; row_idx++) {
        ...
        if (path_val.IsNull() || band_val.IsNull() || row_val.IsNull() || col_val.IsNull()) {
        ...
        try {
            ...
            CPLErr err = band->RasterIO(GF_Read, c, r, 1, 1, &val, 1, 1, GDT_Float64, 0, 0);
            if (err != CE_None) {
                result.SetValue(row_idx, Value());
            } else {
...
static void RasterCellRowFunction(DataChunk &input, ExpressionState &, Vector &result) {
    auto count = input.size();
    result.SetVectorType(VectorType::FLAT_VECTOR);
    for (idx_t row_idx = 0; row_idx < count; row_idx++) {
...
static void RasterCellColFunction(DataChunk &input, ExpressionState &, Vector &result) {
    auto count = input.size();
    result.SetVectorType(VectorType::FLAT_VECTOR);
    for (idx_t row_idx = 0; row_idx < count; row_idx++) {
        Value cell_val = input.data[0].GetValue(row_idx);
...
static void RasterTemporalMeanFunction(DataChunk &input, ExpressionState &, Vector &result) {
    auto count = input.size();
    result.SetVectorType(VectorType::FLAT_VECTOR);
    for (idx_t row_idx = 0; row_idx < count; row_idx++) {
...
//  large datasets.
static void RasterBandSumFunction(DataChunk &input, ExpressionState &, Vector &result) {
    auto count = input.size();
    result.SetVectorType(VectorType::FLAT_VECTOR);
    for (idx_t row_idx = 0; row_idx < count; row_idx++) {
        Value path_val = input.data[0].GetValue(row_idx);
        ...
        try {
            ...
            for (int y = 0; y < height; y++) {
                CPLErr err = band->RasterIO(GF_Read, 0, y, width, 1, buffer.data(), width, 1, GDT_Float64, 0, 0);
                if (err != CE_None) {
...
struct CubeBindData : public FunctionData {
    ...
    bool Equals(const FunctionData &other) const override {
        ...
        return paths == o.paths && vars == o.vars && times == o.times && width == o.width && height == o.height;
    }
};
...
// Bind for read_raster_cube: parse three lists and ensure all files have same dimensions
static unique_ptr<FunctionData> CubeBind(ClientContext &, TableFunctionBindInput &input,
                                         vector<LogicalType> &types, vector<string> &names) {
    if (input.inputs.size() < 3) {
        throw BinderException("read_raster_cube(paths, vars, times) requires three arguments");
    }
    std::vector<std::string> paths;
    std::vector<std::string> vars;
    std::vector<std::string> times;
    ...
    if (paths.size() != vars.size() || paths.size() != times.size()) {
        throw BinderException("paths, vars and times lists must have the same length");
    }
    ...
    for (size_t i = 0; i < paths.size(); i++) {
        ...
        if (i == 0) {
            ...
            if ((idx_t)w != width || (idx_t)h != height) {
                throw BinderException("All rasters in read_raster_cube must have identical dimensions");
            }
        }
    }
    types = {LogicalType::BIGINT, LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::DOUBLE};
...
static unique_ptr<GlobalTableFunctionState> CubeInit(ClientContext &, TableFunctionInitInput &in) {
...
static void CubeScan(ClientContext &, TableFunctionInput &in, DataChunk &out) {
    ...
    while (out_idx < STANDARD_VECTOR_SIZE && st.dataset_index < st.ds.size()) {
        ...
        if (st.dataset_index < st.ds.size()) {
            ...
            if (band) {
                CPLErr err = band->RasterIO(GF_Read, (int)c, (int)r, 1, 1, &val, 1, 1, GDT_Float64, 0, 0);
                if (err == CE_None) {
                    ok = true;
                }
            }
...
static unique_ptr<FunctionData> MetaBind(ClientContext &, TableFunctionBindInput &input,
                                         vector<LogicalType> &types, vector<string> &names) {
    ...
    types = {LogicalType::BIGINT, // width
    ...
    names = {"width", "height", "bands", "origin_x", "origin_y", "pixel_x", "pixel_y", "srs"};
...
static unique_ptr<GlobalTableFunctionState> MetaInit(ClientContext &, TableFunctionInitInput &in) {
...
void RegisterRasterFunctions(DatabaseInstance &db) {
    ...
    TableFunction meta_tf("raster_metadata", {LogicalType::VARCHAR}, MetaScan, MetaBind, MetaInit);
    ExtensionUtil::RegisterFunction(db, meta_tf);

    // NDVI
    ScalarFunction ndvi_fn("ndvi", {LogicalType::DOUBLE, LogicalType::DOUBLE}, LogicalType::DOUBLE, NdviFunction);
    ExtensionUtil::RegisterFunction(db, ndvi_fn);

    // trend over a list of doubles
    ScalarFunction trend_fn("trend", {LogicalType::LIST(LogicalType::DOUBLE)}, LogicalType::DOUBLE, TrendFunction);
    ...
    ScalarFunction pixel_fn("raster_pixel_value",
                           {LogicalType::VARCHAR, LogicalType::INTEGER, LogicalType::BIGINT, LogicalType::BIGINT},
                           LogicalType::DOUBLE, RasterPixelValueFunction);
    ExtensionUtil::RegisterFunction(db, pixel_fn);

    // cell_id conversion helpers
    ScalarFunction cell_row_fn("raster_cell_row", {LogicalType::BIGINT, LogicalType::BIGINT}, LogicalType::BIGINT,
                               RasterCellRowFunction);
    ExtensionUtil::RegisterFunction(db, cell_row_fn);
    ScalarFunction cell_col_fn("raster_cell_col", {LogicalType::BIGINT, LogicalType::BIGINT}, LogicalType::BIGINT,
                               RasterCellColFunction);
    ExtensionUtil::RegisterFunction(db, cell_col_fn);

    // temporal mean: mean of a list of doubles
    ScalarFunction tmean_fn("raster_temporal_mean", {LogicalType::LIST(LogicalType::DOUBLE)}, LogicalType::DOUBLE,
                            RasterTemporalMeanFunction);
    ExtensionUtil::RegisterFunction(db, tmean_fn);

    // band sum: sum of all values in a band
    ScalarFunction bsum_fn("raster_band_sum", {LogicalType::VARCHAR, LogicalType::INTEGER}, LogicalType::DOUBLE,
                           RasterBandSumFunction);
    ExtensionUtil::RegisterFunction(db, bsum_fn);

    // read_raster_cube table function
    TableFunction cube_tf("read_raster_cube", {LogicalType::ANY, LogicalType::ANY, LogicalType::ANY}, CubeScan,
                          CubeBind, CubeInit);
